...
class Linear_BBB(nn.Module):
    ...
    def __init__(self, input_features, output_features, prior_var= [torch.tensor(3/4), torch.tensor(np.exp(-1)), torch.tensor(np.exp(-7)) ]):
        ...
        '''
        # initialize weight params
        self.w_mu = nn.Parameter(torch.zeros(output_features, input_features).normal_(0, 0.1))
        self.w_rho = nn.Parameter(torch.zeros(output_features, input_features).normal_(-5,0.1))

        #initialize bias params
        self.b_mu =  nn.Parameter(torch.zeros(output_features).normal_(0, 0.1))
        self.b_rho = nn.Parameter(torch.zeros(output_features).uniform_(-5,0.1))
        '''
        # initialize prior distribution
        self.prior = GMMPrior(prior_var)
        #self.prior = GaussianPrior(1)
